    ...
    try {
      ...
      if (!bypassCooldown && lastSync) {
        ...
        if (timeElapsed < cooldownMs) {
          const minutesLeft = ((cooldownMs - timeElapsed) / (60 * 1000)).toFixed(0);
          const hoursElapsed = (timeElapsed / (60 * 60 * 1000)).toFixed(2);
          this.logger.log(`Sync requested, but skipped due to cooldown. Last sync was ${hoursElapsed} hours ago. (${minutesLeft} mins remaining).`);
