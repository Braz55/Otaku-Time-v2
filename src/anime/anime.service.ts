...
export class AnimeService {
  ...
  async getRecommendations(type: 'ANIME' | 'MANGA', userId: number, page: number = 1) {
    let userLibraryIds = new Set<number>();
    let userInteractedItems: { generos: any; status: string; paisOrigem: string | null }[] = [];

    if (type === 'ANIME') {
      const userAnimes = await this.prisma.userAnime.findMany({ where: { userId }, include: { anime: true } });
      ...
      const userMangas = await this.prisma.userManga.findMany({ where: { userId }, include: { manga: true } });
    ...
    userInteractedItems.forEach(item => {
      ...
      if (typeof item.generos === 'string') {
        item.generos.split(',').map(s => s.trim()).filter(Boolean).forEach(g => { genresObj[g] = 100; });
    ...
    const allDbGenresTags = await this.prisma.genreTag.findMany({ select: { name: true, type: true } });
    const dbGenreNames = allDbGenresTags.filter(gt => gt.type === 'GENRE').map(gt => gt.name);
    ...
    userInteractedItems.forEach(item => {
      if (item.paisOrigem) {
        countryCounts[item.paisOrigem] = (countryCounts[item.paisOrigem] || 0) + 1;
      }
    });
    ...
    const buildProfileForAnchor = (anchor: string, excludeTastes: string[] = []) => {
      if (!anchor) return [];
    ...
    const profile1 = sortedTastes[0] ? buildProfileForAnchor(sortedTastes[0]) : [];
    const profile2 = sortedTastes[1] ? buildProfileForAnchor(sortedTastes[1], profile1) : [];
    const profile3 = sortedTastes[2] ? buildProfileForAnchor(sortedTastes[2], [...profile1, ...profile2]) : [];

    console.log(`\x1b[35m[Recomendações - ${type}]\x1b[0m Utilizador ID: ${userId}`);
    console.log(`  -> País de preferência (Preferred Country):`, preferredCountry || 'Nenhum');
    ...
    const fetchCandidatePool = async (
      ...
      return this.fetchCandidatesFromAniList(type, genres, tags, sort, isAdult, country);
    };

    const [poolPrimary1, poolPrimary2, poolSecondary1, poolDiscovery, poolGlobal] = await Promise.all([
      profile1.length > 0 ? fetchCandidatePoolForProfile(profile1, 'TRENDING_DESC', preferredCountry) : Promise.resolve([]),
      profile2.length > 0 ? fetchCandidatePoolForProfile(profile2, 'POPULARITY_DESC', preferredCountry) : Promise.resolve([]),
      profile3.length > 0 ? fetchCandidatePoolForProfile(profile3, 'POPULARITY_DESC', preferredCountry) : Promise.resolve([]),
      discoveryTastes[0] ? fetchCandidatePool(dbGenreNames.includes(discoveryTastes[0]) ? [discoveryTastes[0]] : undefined, !dbGenreNames.includes(discoveryTastes[0]) ? [discoveryTastes[0]] : undefined, 'TRENDING_DESC') : Promise.resolve([]),
      fetchCandidatePool(undefined, undefined, 'TRENDING_DESC', preferredCountry)
    ]);
